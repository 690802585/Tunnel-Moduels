        ...
        for i in range(0,n-m+1):#对所有回路循环
            ...
            for j in range(0,n):#对所有分支循环
                ...
                if(fanindex==1):
                    ...
                    Hf[j]=0
                f=f+C[i,j]*(inp[j,3]*Q[j]*abs(Q[j])+PW[i]-Hf[j]-inp[j,5])#inp矩阵第4列为风阻，第6列为温升压力(暂不考虑)
                df=df+(abs(C[i,j])*2*inp[j,3]*abs(Q[j])-Bv-2*Av*Q[j])
            dq=-f/df#计算回路风量修正值
            for j in range(0,n):
                Q[j]=Q[j]+C[i,j]*dq#修正回路风量
            f=0.0#累加前赋初值
            for j in range(0,n):
                f=f+C[i,j]*(inp[j,3]*Q[j]*abs(Q[j])+PW[i]-Hf[j]-inp[j,5])#重新计算回路不平衡风压
